...
class PerfilProvider extends ChangeNotifier {
  ...
  Future<void> cargarPerfil(int usuarioId) async {
    _status = PerfilStatus.cargando; _error = null;
    notifyListeners();
    try {
      _perfil = await _repo.getPerfil(usuarioId);
      _status = PerfilStatus.cargado;
    } on ApiException catch (e) {
      _error = e.message; _status = PerfilStatus.error;
    } catch (_) {
      _error = 'Error al cargar el perfil.'; _status = PerfilStatus.error;
    }
    notifyListeners();
  }

  // ── Actualizar perfil ─────────────────────────────────────────────────────
  Future<bool> actualizarPerfil(int usuarioId, {
    ...
    notifyListeners();
    try {
      _perfil = await _repo.updatePerfil(usuarioId,
      ...
      _error = 'Error al actualizar el perfil.'; _status = PerfilStatus.error;
      notifyListeners(); return false;
    }
  }
  ...
  Future<bool> subirFoto(int usuarioId, File foto) async {
    _uploadingFoto = true; _error = null; notifyListeners();
    try {
      await _media.uploadFotoEstudiante(usuarioId, foto);
      _perfil = await _repo.getPerfil(usuarioId);
      _status = PerfilStatus.cargado;
      _uploadingFoto = false; notifyListeners(); return true;
    } on ApiException catch (e) {
      _error = e.message; _uploadingFoto = false; notifyListeners(); return false;
    } catch (_) {
      _error = 'Error al subir la foto.'; _uploadingFoto = false; notifyListeners(); return false;
    }
  }
  ...
  Future<bool> eliminarFoto(int usuarioId) async {
    ...
    try {
      ...
      _error = 'Error al eliminar la foto.'; _uploadingFoto = false; notifyListeners(); return false;
    }
  }

  // ── Subir CV ──────────────────────────────────────────────────────────────
  Future<bool> subirCv(int usuarioId, File cv) async {
    _uploadingCv = true; _error = null; notifyListeners();
    try {
      await _media.uploadCvEstudiante(usuarioId, cv);
      _perfil = await _repo.getPerfil(usuarioId);
      _status = PerfilStatus.cargado;
      _uploadingCv = false; notifyListeners(); return true;
    } on ApiException catch (e) {
      _error = e.message; _uploadingCv = false; notifyListeners(); return false;
    } catch (_) {
      _error = 'Error al subir el CV.'; _uploadingCv = false; notifyListeners(); return false;
    }
  }
  ...
  Future<bool> eliminarCv(int usuarioId) async {
    ...
    try {
      ...
      _error = 'Error al eliminar el CV.'; _uploadingCv = false; notifyListeners(); return false;
    }
  }
}